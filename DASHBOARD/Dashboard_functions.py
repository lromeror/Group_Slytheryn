...
def country_flag(IMAGES_DIR,Id_team,i):
    country_flag = dbc.Col([html.Img(src=os.path.join(IMAGES_DIR,Id_team[i]))],md=1)
    return country_flag
...
def rows_countries_flags(IMAGES_DIR,Id_team,list_id_images):
    ...
    country_flag(IMAGES_DIR,Id_team,list_id_images[12])])  
    return  rows_countries

def div_countries (number_rows):